...
const getInitialParams = () => {
  ...
  return {
    // Elysium is music-first: always force music trending
    type: "music" as TrendingFilterType,
    region: (urlParams.get("region") as string) ?? initialState.region,
  };
};

const getCountryCode = async (): Promise<GeoLocation> => {
  const base = process.env.REACT_APP_API_URL ?? "";
  const url = base ? `${base}/api/countryCode` : `${window.location.origin}/api/countryCode`;
  try {
    const request = await fetch(url);
    const data = await request.json();
    return data ?? { city: "", country: "US", countryRegion: "", region: "" };
  } catch {
    return { city: "", country: "US", countryRegion: "", region: "" };
  }
};
