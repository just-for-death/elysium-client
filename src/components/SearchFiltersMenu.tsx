...
import { IconBrandApple, IconFilter } from "@tabler/icons-react";
...
export const SearchFilters = memo(() => {
  ...
  const isAppleMusic = searchValues.service === "apple_music";

  return (
    <>
      {/* Apple Music toggle button — visible in header for quick access */}
      <ActionIcon
        aria-label="Search Apple Music"
        variant={isAppleMusic ? "filled" : "default"}
        radius="md"
        size={36}
        color={isAppleMusic ? "pink" : "gray"}
        title={isAppleMusic ? "Switch to Invidious search" : "Search Apple Music"}
        onClick={() =>
          setSearchValues({
            ...searchValues,
            service: isAppleMusic ? "invidious" : "apple_music",
          })
        }
      >
      ...
      {/* Filter menu — only useful for non-Apple-Music searches */}
      {!isAppleMusic && (
