...
export const ButtonFavorite: FC<ButtonFavoriteProps> = memo(
  ({
    ...
    const cards = favorite?.cards ?? [];
    const isFavorite = cards.find(
    ...
    const updateAndCommit = (updatedFavoritePlaylist: Favorite) => {
      ...
      db.commit();
      const next = getFavoritePlaylist();
      setFavorite({ ...(next ?? {}), cards: next?.cards ?? [] });
    };
    ...
    const handleAdd = () => {
      const formatedCard = (() => {
        switch (card.type) {
          ...
          default:
            return formatedCardVideo(card as FavoriteVideo);
        }
      })();

      updateAndCommit({
        ...favorite,
        cards: [formatedCard, ...cards],
      });
